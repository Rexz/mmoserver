...
#ifndef ANH_THREADEDMESSAGEFACTORY_H
#define ANH_THREADEDMESSAGEFACTORY_H
...
class Message;
class MessageFactoryChunk;
class ThreadedMessageFactory;

#define gThreadedMessageFactory			ThreadedMessageFactory::getSingleton()

// NEVER DELETE MESSAGES THAT ARE STILL REFERENCED SOMEWHERE
#define MESSAGE_MAX_LIFE_TIME	60000
#define CHUNK_COUNT				16

typedef tbb::concurrent_queue<MessageFactoryChunk*>			ChunkQueue;
//======================================================================================================================
...
{
public:
	MessageFactoryChunk(uint32 heapSize, uint8 id, ThreadedMessageFactory* factory);
	~MessageFactoryChunk();
...
private:
	...
	tbb::atomic<int>		mFreeChunk;

	ThreadedMessageFactory*	mFactory;
...
class ThreadedMessageFactory
{
	friend MessageFactoryChunk;
public:

	

    ThreadedMessageFactory(uint32 heapSize, int chunks);
    ~ThreadedMessageFactory();

	MessageFactoryChunk*			ThreadedMessageFactory::getChunk();
	bool							getBusy(void);
    

    static ThreadedMessageFactory*	getSingleton(void);
	static ThreadedMessageFactory*	getSingleton(uint32_t heap_size, uint32 chunks);
...
private:
	...
	/**
	* returns the chunk to the chunkQueue for a new message to snatch it up
    * possibly implement a returnfunction for chunks that have full heaps
	* and thus are temporarily inaccessible 
	*
	* \param chunk  the chunk returned
	*
	*/
	void returnChunk(MessageFactoryChunk* chunk);
	void returnBusyChunk(MessageFactoryChunk* chunk);

    MessageFactoryChunk*	mChunk;
	int						mChunkCount;

	ChunkQueue				mChunkQueue;
	ChunkQueue				mBusyChunkQueue;

    static ThreadedMessageFactory*	mSingleton;
    // Anh_Utils::Clock*		mClock;
};

//======================================================================================================================

inline ThreadedMessageFactory* ThreadedMessageFactory::getSingleton(void)
{
    if(!mSingleton)
    {
...
inline ThreadedMessageFactory* ThreadedMessageFactory::getSingleton(uint32_t heap_size, uint32 chunks)
{
    if(!mSingleton)
    {
        mSingleton = new ThreadedMessageFactory(heap_size*1024, chunks);
    }
...
inline void ThreadedMessageFactory::destroySingleton(void)
{
    if (mSingleton)
    {
        delete mSingleton;
...
inline uint32 MessageFactoryChunk::_getHeapSize(void)
{
...
#endif  //MMOSERVER_THREADEDMESSAGEFACTORY_H
